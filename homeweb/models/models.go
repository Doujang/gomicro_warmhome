...
/* 用户 table_name = user */
type User struct {
	Uid           string        `orm:"pk;size(256)" json:"user_id"`   //用户编号
	Name          string        `orm:"size(50)"  json:"name"`         //用户昵称
	Password_hash string        `orm:"size(256)" json:"password"`     //用户密码加密的
	Email         string        `orm:"size(50);unique"  json:"email"` //邮箱
